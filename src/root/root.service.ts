...
export class RootService {
    ...
    private async createDBIfNotExist() {
            ...
            let request: string = 'CREATE TABLE IF NOT EXISTS client(id INTEGER PRIMARY KEY AUTOINCREMENT, guid TEXT, first TEXT, last TEXT, street TEXT, city TEXT, zip NUMERIC);'
            db.prepare(request).run()
            db.close()
    }

    private async insertDataFromCsv() {
        const db = new Database('./db/SQLite.db'/*, { verbose: this.logger.log }*/)
